...
export const TrendDetail = ({ post, onBack }: { post: Post; onBack: () => void }) => {
  ...
  const copyLink = async (type: 'tg' | 'web') => {
    const link =
      type === 'tg'
        ? `https://t.me/${bot?.bot_username || 'bot'}?startapp=post-${post.id}${userId ? `_ref-${userId}` : ''}`
        : `${window.location.origin}/trend/${post.id}${userId ? `?ref=${userId}` : ''}`;
  ...
  return (
    <div className="min-h-svh bg-[#05070b] text-white">
      <header className="sticky top-0 z-50 flex items-center gap-3 border-b border-white/10 bg-black/35 px-5 py-4 backdrop-blur-3xl">
        <button
          onClick={onBack}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-white/10 bg-black/70 px-5 pb-[calc(18px+max(12px,env(safe-area-inset-bottom)))] pt-4 backdrop-blur-3xl">
      ...
      <Dialog open={isShareOpen} onOpenChange={setIsShareOpen}>
        <DialogContent className="max-w-md rounded-[32px] border-white/10 bg-zinc-950/90 p-6 text-white backdrop-blur-3xl">
          <DialogHeader>
            <DialogTitle className="text-[20px] font-black">{t('shareTrend')}</DialogTitle>
          ...
          <div className="mt-2 flex flex-col gap-3">
            <ShareButton
              ...
              title={t('tgAppLink')}
              description={t('tgAppLinkDesc')}
              active={copiedType === 'tg'}
              onClick={() => copyLink('tg')}
            />
            <ShareButton
              icon={<Globe className="size-5" />}
              title={t('webBrowserLink')}
              description={t('webBrowserLinkDesc')}
              active={copiedType === 'web'}
              onClick={() => copyLink('web')}
            />
          </div>
        </DialogContent>
      </Dialog>
...
const ShareButton = ({
  ...
  <button
    onClick={onClick}
    className="flex items-center gap-4 rounded-3xl border border-white/10 bg-white/7 p-4 text-left transition active:scale-[0.98]"
  >
    ...
    <div className="min-w-0 flex-1">
      ...
      <p className="mt-1 truncate text-[12px] font-semibold text-white/40">{description}</p>
    </div>
    <div className="grid size-9 place-items-center rounded-xl border border-white/10 bg-black/20 text-white/50">
      {active ? <CheckCircle2 className="size-5 text-cyan-100" /> : <Copy className="size-4" />}
    </div>
  </button>
);
